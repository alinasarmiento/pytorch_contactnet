...
class PandaPB():
    ...
    def __init__(self, model, dataset, config, gui=True):
        ...
        self.cn_path = os.path.join(os.getenv('HOME'), 'pytorch_contactnet/')
        self.acr_path = os.path.join(os.getenv('HOME'), 'acronym/')
    ...
    def get_rand_scene(self, scene_idx):
        ...
        filename = self.acr_path + 'scene_contacts/' + os.fsdecode(data_file)
        scene_data = np.load(filename, allow_pickle=True)
        unfixed_obj_paths = scene_data['obj_paths']
        obj_paths = []
        for i, path in enumerate(unfixed_obj_paths):
            path = self.fix_path(path.split('/')[-1])
            fixed_path = self.acr_path + 'models/' + path
            print('in get rand scene', fixed_path)
            obj_paths.append(fixed_path)
    ...
    def generate_scene(self, scene_index=None):
        ...
        for path, scale, transform in zip(obj_paths, obj_scales, obj_transforms):
            name = self.cn_path + 'obj-models/' + 'object_' + str(i) + '.obj'
            i += 1
            transform[2, 3] -= 0.3
    ...
    def pb_execute(self, args):
        ...
        for obj_grasps, obj_widths, obj_points, s, seg_id in zip(grasps, widths, points, success, seg_ids):
            ...
            obj_success = s[sample_idx]
            print('object:', seg_id)
            for grasp, width, point, success in zip(obj_grasps, obj_widths, obj_points, obj_success):
                if success < 0.03:
                    print('not very confident in this grasp. confidence is:', success)
                    continue
