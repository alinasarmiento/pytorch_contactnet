...
import config_utils
from data_utils import regularize_pc_point_count, depth2pc, load_available_input_data
...
def inference(global_config, checkpoint_dir, input_paths, K=None, local_regions=True, skip_border_objects=False, filter_grasps=True, segmap_id=None, z_range=[0.2,1.8], forward_passes=1):
    ...
    (as i understand it, this is a full forward pass on one cluttered scene)
    ...
    # Build the model
    contactnet = ContactNet(global_config)

    # Load weights
    contactnet.load_weights(checkpoint_dir, mode='test')
    ...
    for p in glob.glob(input_paths):
        ...
        if pc_full is None:
            ...
            pc_full, pc_segments, pc_colors = contactnet.extract_point_clouds(depth, cam_K, segmap=segmap, rgb=rgb,
                                                                                    skip_border_objects=skip_border_objects, z_range=z_range)

        print('Generating Grasps...')
        pred_grasps_cam, scores, contact_pts, _ = contactnet.predict_scene_grasps(sess, pc_full, pc_segments=pc_segments, 
